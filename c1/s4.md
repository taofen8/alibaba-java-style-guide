...
5. `taofen8【强制】` Deprecated 方法必须强制添加以下java doc注释
```java
/**
...
7.**【强制】** Object的equals方法容易抛空指针异常，应使用常量或确定有值的对象来调用equals。
```java
...
8.**【强制】** 所有的相同类型的包装类对象之间值的比较，全部使用equals方法比较。
...
17 `taofen8 强制` 在getter/setter方法中(get+成员变量名，set+成员变量名)，一律不能增加业务逻辑。
```java
反例：
...
18.*【推荐】* 循环体内，字符串的联接方式，使用StringBuilder的append方法进行扩展。
```java
反例：
String str = "start";
for(int i=0; i<100; i++){
...
19.*【推荐】*final可提高程序响应效率，声明成final的情况：
